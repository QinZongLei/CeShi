...
-(void)func_setMenuTableViewCell{
    ...
    [self.contentView addSubview:({
        ...
        self.iv_iconImage.frame = CGRectMake(kWidth(40), kWidth(10), kWidth(30), kWidth(30));
        self.iv_iconImage;
    })];
    
    [self.contentView addSubview:({
        self.lb_name = [[UILabel alloc] init];
        self.lb_name.textColor = color_agreement_tip;
        self.lb_name.frame = CGRectMake(self.iv_iconImage.right+kWidth(5), self.iv_iconImage.top, kWidth(70), kWidth(30));
    ...
    [self.contentView addSubview:({
        ...
        self.lb_prompt.frame = CGRectMake(self.lb_name.right+kWidth(2), self.iv_iconImage.top, kWidth(100), kWidth(30));
        self.lb_prompt;
    })];
    
    [self.contentView addSubview:({
        self.btn_operation = [UIButton buttonWithType:UIButtonTypeCustom];
        [self.btn_operation setBackgroundImage:kSetBundleImage(operationImg) forState:UIControlStateNormal];
        [self.btn_operation addTarget:self action:@selector(func_clickAction:) forControlEvents:UIControlEventTouchUpInside];
        self.btn_operation.frame = CGRectMake(self.lb_prompt.right+kWidth(45), kWidth(5), kWidth(80), kWidth(40));
        self.btn_operation;
    })];
}
...
- (void)setMenuDic:(NSDictionary *)menuDic{
    ...
    if ([@"(已实名认证)" isEqualToString:promptStr] || [@"(手机已绑定)" isEqualToString:promptStr]) {
        self.btn_operation.hidden = YES;
        self.lb_prompt.textColor = [UIColor greenColor];
    }
}
