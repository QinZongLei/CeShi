...
-(void)func_setIdCardCertificationView{
    ...
    [self.iv_viewBg addSubview:({
        ...
        self.lb_prompt.frame = CGRectMake((self.iv_viewBg.width-(self.iv_viewBg.width-kWidth(120)))/2, self.iv_line.bottom + kWidth(10), self.iv_viewBg.width-kWidth(100), kWidth(0));
    ...
    [self.iv_viewBg addSubview:({
        self.view_idCard =[[SimpleSDK_CustomUITextFieldView alloc] initWithFrame:CGRectMake(self.view_nikeNmae.left, self.view_nikeNmae.bottom+kWidth(15), kWidth(300), kWidth(35))];
    ...
    [self.iv_viewBg addSubview:({
        ...
        self.btn_certification.frame = CGRectMake((self.iv_viewBg.width-kWidth(150))/2, self.view_idCard.bottom+kWidth(15), kWidth(180), kWidth(40));

        self.btn_certification.tag = 20211202;
        [self.btn_certification setBackgroundImage:kSetBundleImage(determineBtn) forState:UIControlStateNormal];
...
- (void)setShowType:(NSString *)showType
{
    ...
    if (![@"3" isEqualToString: self.showType]) {
        [self.iv_viewBg addSubview:({
            ...
            self.btn_changeAcount.frame = CGRectMake(self.iv_viewBg.width-kWidth(110), self.btn_certification.bottom - kWidth(15), kWidth(80), kWidth(20));
            [self.btn_changeAcount setTitle:@"切换账号" forState:0];
            self.btn_changeAcount.titleLabel.textAlignment = NSTextAlignmentCenter;
            
            [self.btn_changeAcount setAttributedTitle:[SimpleSDK_Tools func_strAddUnderline:@"切换账号" UnderLineColor:[UIColor ZNWinObj_colorWithHexString:@"#fe0001"]] forState:0];

            self.btn_changeAcount.titleLabel.font = [UIFont systemFontOfSize:kWidth(16)];
...
- (void)func_changeAction:(UIButton *)sender {
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        [SimpleSDK_ApiManager func_logout:^(BOOL status) {
            if (self) {
                [self removeFromSuperview];
                STARTLOGIN([SimpleSDK_Expose sharedInstance].loginHandle,[SimpleSDK_Expose sharedInstance].logoutHandle);
            }
        }];
    });
