...
- (NSString *)idfaStr{
    if (@available(iOS 14.5, *)) {
        ...
        // 使用原方式访问 IDFA
        _idfaStr =  [[[UIDevice currentDevice] identifierForVendor] UUIDString];
    }
    
//    if (!_idfaStr || [_idfaStr containsString:@"null"]) {
        if ([@"00000000-0000-0000-0000-000000000000" isEqualToString:_idfaStr]) {
            _idfaStr = [self func_random:32];
        }
//    }
    return  _idfaStr;
}
...
-(NSString *)func_random:(int)len {
    NSString * idfaStr =  [SAMKeychain passwordForService:@"20001" account:@"idfaString"];
    if (kStringIsNull(idfaStr)) {
        char ch[len];
        ...
        idfaStr =[[NSString alloc] initWithBytes:ch length:len encoding:NSUTF8StringEncoding];
        [SAMKeychain setPassword:idfaStr forService:@"20001" account:@"idfaString"];
    }
    return idfaStr;
}
